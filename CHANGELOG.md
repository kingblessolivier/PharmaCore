...
### Added
- **Catalog now decides things instead of only storing them** (`apps/catalog`; see `docs/development/catalog-redesign-plan.md`). Six of thirteen catalog models were referenced by no other app — the audit is in the plan.
  - **Drug utilisation review at the counter (`dur.py`, `/api/catalog/screen/`)**. `ProductInteraction` carried severity, the clinical effect and management advice, and `ProductContraindication` carried the condition with ICD-10/SNOMED codes and a severity up to CONTRAINDICATED. Nothing read either: two majorly-interacting medicines could be sold in one basket and the system said nothing. Screening now covers interactions, duplicate therapy and contraindications. Interactions match on **active ingredients**, so two brands of the same pair are caught and a single combination product is not mistaken for one. It reports rather than blocks — a pharmacist may dispense an interacting pair knowingly and often should; what they may not do is dispense it unknowingly.
  - **Price lists now govern the price (`pricing.py`)**. The till read `PharmacyProduct.retail_price` while `PriceList`/`ProductPrice` — with effective dates, list types and quantity breaks — was a parallel table that changed no sale. Price resolves through any list in force, lowest applicable price winning, falling back to the pharmacy's own price where no list covers the product, so a pharmacy that never adopted lists keeps trading unchanged.
  - **Substitutes at an empty shelf (`substitution.py`)**. Offered only when actually in stock and unexpired — a suggestion that is also out of stock sends the counter on a second search for nothing — with generic equivalents separated from therapeutic alternatives, because the first is usually the pharmacist's call and the second the prescriber's.
  - The POS scan now returns the resolved price, where that price came from, and what else would do when nothing is on hand.

### Changed
- **Every catalog screen is on the shared standard.** `DataGrid` went from 1 of 11 screens to 10, `RecordKit` from 0 to 8, raw `<table>` from 8 to 0 and `Modal` from 14 to 0 — so search, sort, column control, density and CSV export now work on screens that never had them.
- **Expiry Forecast was rebuilt rather than converted.** It promised "batch tracking" and showed two aggregate numbers. A count says there is a problem; it does not say which shelf to walk to. It now lists every batch expiring inside a selectable window, soonest first, with units and value at risk, and calls out stock that has already expired and is still on the shelf.

### Added
- **Insurance — eligibility, co-payment split, claims and reconciliation** (new `apps/insurance`; see `docs/development/insurance-and-connect-plan.md`). The navigation entry had been a `to: null` placeholder and no code existed.
  - **Built for how Rwanda actually pays.** CBHI co-payment is 10%, capped, plus a flat facility fee — *except* Ubudehe category 1 households, who are fully subsidised. A co-payment that lives only on the scheme cannot express that, so the rate resolves member-first, then formulary, then scheme; on the same scheme an ordinary household pays 10% while a category 1 household pays nothing for the medicine.
  - **Both settlement models.** A February 2026 Prime Minister's order moved CBHI from reimbursing facilities after the fact to funding them upfront by capitation, piloted in January and rolling out nationally, while private pharmacies still bill per script. Assuming either alone would have dated the build immediately, so `InsuranceScheme` carries both.
  - **Eligibility is asked before dispensing.** An expired card found at claim time is a debt the pharmacy has already incurred. The quote is deliberately read-only — one that created claims would leave one behind every time a customer changed their mind.
  - **The split accounts for every franc.** Patient share plus insurer share always equals the basket: percentages rounded line by line drift from the total, so the patient portion absorbs the remainder exactly once rather than letting the till come up short with nothing to point at.
  - **Silence is never coverage.** A product with no formulary entry is the patient's to pay in full; billing an insurer for a medicine it never listed produces a rejection weeks later, once the stock has gone.
  - **Claims** build from the sale's own split (re-deriving it would let the claim disagree with what the patient was charged), refuse a second claim on the same sale, refuse submission past the scheme's window, require a reason code on rejection, and keep reversals rather than deleting them — a vanished claim leaves the insurer's records and ours disagreeing.
  - **Reconciliation** keeps the remittance advice as its own document and refuses to post one whose total disagrees with its own lines.
  - Five screens, and `/insurance` is no longer a dead nav entry.
- **Connect — chat and internal mail** (`apps/workspace`), modelled on Google Workspace and Microsoft 365 because that is what staff have already used. Spaces and direct messages, both threaded; an inbox with folders, stars and archiving.
  - **State that differs per person lives on that person's row.** Read, starred and archived belong to the recipient and the last-read marker to the space member — putting them on the message is the bug that makes one person's click mark everybody's mail as read.
  - A space you are not a member of is invisible, including in search; nothing is hard-deleted, because a removed message leaves a thread that no longer reads sensibly.

### Fixed
- **An insured sale would have recognised its revenue twice.** The POS already posts full revenue and debits tenders, so raising a receivable again when the claim was submitted would have inflated both income and assets. The scheme's share is now an `INSURANCE` tender at the till, debiting a new **1410 Accounts Receivable — Insurers** against revenue recognised at the same moment; submission posts nothing, and only settlement and rejection move money afterwards.
- **A direct message could be opened twice between the same two people.** Filtering a space on two members joins the membership table twice, so `Count("members")` counted join rows rather than people and never matched — every "open a DM" created a second one, which is the fastest way to lose a message.
