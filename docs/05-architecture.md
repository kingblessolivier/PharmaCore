...
**Decision:** A single deployable **modular monolith** backend (Django + DRF) with
...
|---|---|
| **Backend (Django + DRF)** | REST API, business logic, module apps, RBAC, audit, admin |
...
|---|---|---|---|
| Backend | **Python 3.12 + Django + DRF** | Batteries-included (admin/auth/ORM), typed via stubs, great for a CRUD-heavy ERP | Less async-native than FastAPI — fine at this scale |
| ORM/migrations | **Django ORM + migrations** | Integrated, mature, admin-backed, autogenerated | Less explicit than SQLAlchemy |
