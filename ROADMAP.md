...
- ✅ **Reorder management** — min/max/reorder points, par levels, safety stock sized to a service level, suggested orders netting off reservations, **ABC/XYZ** matrix, slow/dead/never‑moved stock with capital tied up, near‑expiry action lists that narrow as the clock runs (`ReorderRule`, `analytics.py`, `/inventory/replenishment`).
...
- ✅ **GS1 2D DataMatrix** scan & parse — element‑string, bracketed and **Digital Link** forms, FNC1‑correct variable‑length AIs, mod‑10 check digits verified (a mis‑scan is rejected, not stored); per‑unit **serialisation & track‑&‑trace** with each→case→pallet **aggregation**, cascading business steps, chain‑of‑custody trace and **EPCIS 2.0 JSON‑LD export** (`gs1.py`, `serialisation.py`, `SerialUnit`, `EpcisEvent`, `/inventory/serialisation`).
- ✅ **Cold‑chain rigour** — **mean kinetic temperature** (Arrhenius‑weighted), calibrated‑sensor register with device identity/accuracy/certificate trail (a fail withdraws the probe rather than extending its due date), excursion investigation over a window with auto‑quarantine and a QA‑signed **disposition** that is what actually releases or holds the lots (`coldchain.py`, `SensorCalibration`, `ExcursionInvestigation`, `/inventory/coldchain`).
- ✅ **Consignment / vendor‑managed inventory (VMI)** — supplier‑owned stock held by us and our stock held at a customer, with title transfer, liability holder and credit limit on the agreement; consumption writes the liability at lot cost and **settlement raises the AP bill and posts it to the GL** (`ConsignmentAgreement`, `ConsignmentConsumption`, `ConsignmentSettlement`, `/inventory/consignment`).
- ✅ **Multi‑warehouse per org** (main store, cross‑dock, cold store, bonded, quarantine, dispensary) with per‑zone occupancy; **put‑away by rule** in priority order, falling back to the label storage condition so cold‑chain stock is never defaulted onto an ambient shelf; **wave/zone picking** — FEFO tasks split across batches, walk‑ordered by aisle/shelf/bin, stock reserved on release and short picks recorded as short (`Warehouse`, `PutawayRule`, `PickWave`, `PickTask`, `warehouse_services.py`, `/inventory/warehouses`, `/inventory/putaway`, `/inventory/picking`).
...
- 🚧 **Investment & equity** — capital invested, owner's equity, drawings, retained earnings, capital‑vs‑expense classification, fixed‑asset register + depreciation. *(Register and disposal were already there; the periodic **depreciation charge is now posted** — `run_depreciation`, `6500` / `1701`, straight‑line, capped at the depreciable amount, idempotent per month. Until this, assets sat at cost until disposal and EBITDA's depreciation add‑back was structurally zero. Owner's drawings and capital‑vs‑expense classification are still open.)*
...
- ✅ **Budgets & costing** — **cost centres** as a tree (branch / department / function / project) tagged onto every journal line, so the ledger can be read per branch without an account per branch; **budgets** as header + lines (account × cost centre × month, annual lines pro‑rated), approved and locked; **budget vs actual with actuals read from the ledger** — the old editable `actual_amount` column is gone, because a variance whose actual is typed in by the person being measured is not a control; variance signed correctly per account type; **cost‑centre P&L** reporting contribution and `tagged_pct` (how much of the P&L is actually coded). See `docs/development/finance-redesign-plan.md`.
...
### Inventory ✅
- ✅ Batch stock, immutable `stock_movements` ledger, **FEFO**, supplier intake (depot‑only), adjustments, wastage, **batch source** (recall traceability), expiry/low‑stock on dashboard, **movement‑history** screen.
- ✅ Multi‑**warehouse** + storage **zones & bins**, **put‑away rules**, **wave/zone picking**, **replenishment intelligence** (reorder points, ABC/XYZ, slow‑dead, near‑expiry), **GS1 serialisation + EPCIS 2.0 track‑&‑trace**, **consignment/VMI** with GL settlement, **cold‑chain rigour** (MKT, calibration register, excursion investigations), **quarantine/recall**, physical **counts**, witnessed **disposal**.
...
- **Phase 4 — Warehouse depth & QMS** 🚧 — ✅ storage **zones/bins**, multi‑warehouse, **put‑away & wave picking**, **temperature + excursion investigations**, **equipment calibration register**, **quarantine/recall**, stock counts, **reorder/replenishment**, disposal, **consignment/VMI**, **GS1 serialisation & EPCIS**. ⬜ the rest of the **Quality Management System** (deviations, CAPA, change control, self‑inspection, supplier qualification) for GDP. *(Inventory → true WMS: done.)*
