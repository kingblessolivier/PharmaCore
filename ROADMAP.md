...
- 🚧 **Password policy, credential & session controls** — ✅ admin **password reset** (forces a change on next sign‑in via `must_change_password`), ✅ user **self‑service change**, ✅ **strength validation** on both, ✅ **force‑logout / session revocation** (token‑version invalidates all a user's access + refresh tokens at once), all audited. ⬜ MFA (authenticator TOTP), API keys / service accounts, SSO (needs an external IdP).
