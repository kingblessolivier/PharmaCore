...
- 🚧 **Password policy & credential controls** — ✅ admin **password reset** (forces a change on next sign‑in via `must_change_password`), ✅ user **self‑service change**, ✅ **strength validation** on both (Django validators), all audited. ⬜ MFA, session controls / force‑logout, API keys / service accounts, SSO.
- ✅ **Admin oversight & monitoring (super‑admin).** The admin can **see and do everything, across every branch and every user** — while RBAC still confines everyone else strictly to their own scope. *(Testing intent: you sign in only as admin, yet must verify each other role sees/does exactly what it should — no more, no less.)*
  ...
  - ✅ **User management** — create / edit / suspend / reactivate users, assign roles & scope (PF number + org), **reset credentials** (force‑change). ⬜ force‑logout (needs the shared auth pipeline), required‑document onboarding is done via [user documents].
